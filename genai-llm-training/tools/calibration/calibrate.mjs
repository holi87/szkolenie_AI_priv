...
export function calibrate(pilot, ctx) {
  ...
  // Poza zakresem: tylko pytania NIEkrytyczne (krytyczne są 100%-bramkowane/konserwatywne — wysoki odsetek
  // poprawnych jest zamierzony; dla nich liczy się niejasność, nie reklasyfikacja trudności — wymagania/07).
  ...
  // Golden set: validated tylko gdy wszystkie pokryte golden są w zakresie i mają niejasność <= 5%.
  ...
  const goldenStatus = {
    ...
    validated: goldenCovered.length > 0 && goldenOffenders.length === 0,
  };

  return {
    coverage: { covered: perQuestion.length, unknownIds },
    perQuestion, outOfBand, criticalsToRewrite, goldenStatus,
    summary: {
...
export function renderReport(pilot, result) {
  ...
  L.push(`- Pokrycie w pilotażu: ${gs.coveredCount}/${gs.totalGolden}`);
...
function main(argv) {
  ...
  if (selfTest) {
    ...
    const checks = [
      ...
      [result.goldenStatus.validated === false, "golden set oznaczony jako wymaga poprawek"],
